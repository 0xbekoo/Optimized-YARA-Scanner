        ...
        if (fd != -1) {
            ...
            if (fstat(fd, &st) == 0 && st.st_size > 0) {
                ...
                if (mapped_data != MAP_FAILED) {
                    madvise(mapped_data, st.st_size, MADV_SEQUENTIAL);
                    #ifdef MADV_HUGEPAGE
                    madvise(mapped_data, st.st_size, MADV_HUGEPAGE);
                    #endif

                    /* Run the YARA Engine */
                    int result = yr_rules_scan_mem(g_rules, (uint8_t*)mapped_data, st.st_size, SCAN_FLAGS_FAST_MODE, \
                        YARACallBack, filepath, 0);
...
int CheckRuleValidity(const char *filepath) {
    ...
    int errors = yr_compiler_add_file(temp_compiler, file, NULL, filepath);
    
    /* Destory the temporary compiler */
    fclose(file);
    yr_compiler_destroy(temp_compiler);
    return (errors == 0);
}
...
int CompileRules(const char *rules_dir) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strstr(entry->d_name, ".yar")) {
            ...
            if (CheckRuleValidity(path)) {
                ...
                if (file) {
                    if (yr_compiler_add_file(compiler, file, NULL, path) == 0) {
                        count++;
                    }
                    fclose(file);
                }
    ...
    printf("[+] The %d number has been determined for the rules\n", count);
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    TraverseDirectory(argv[2]); // Add the all files to the queue
    ...
    printf("Time            : %.4f sn (Wall Clock)\n", duration);
    printf("File Speed      : %.2f dosya/sn\n", speed_files);
    printf("Data Speed      : %.2f MB/sn\n", speed_mb);
    printf("==============================\n");

    if (g_rules) yr_rules_destroy(g_rules);
    yr_finalize();
    free(Threads);
    return 0;
}
